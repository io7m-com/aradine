...
public final class ARCmdInvUninstallSampleMap extends ARCmdAbstract
{
  private static final QParameterNamed1<ARSampleMapID> SAMPLE_MAP_ID =
    new QParameterNamed1<>(
      "--id",
      List.of(),
      new QConstant("The sample map identifier."),
      Optional.empty(),
      ARSampleMapID.class
    );
  ...
  public ARCmdInvUninstallSampleMap()
  {
    super(new QCommandMetadata(
      "uninstall-sample-map",
      new QConstant("Uninstall sample maps from the local inventory."),
      Optional.empty()
    ));
  }
  ...
  {
    return LoggerFactory.getLogger(ARCmdInvUninstallSampleMap.class);
  }
    ...
    throws Exception
  {
    ...
    try (var resources = ARCloseables.create()) {
      ...
      inventory.sampleMapUninstall(
        context.parameterValue(SAMPLE_MAP_ID),
        progress -> {
          logger.info(
            "{} ({}%): {} ({}%)",
            progress.task(),
