...
public final class ARSV1_BRF_Test
{
    ...
    return filterTestCases()
      .map(c -> {
        return DynamicTest.dynamicTest(
          String.format(
            "testFilterCase BRF Q %.2f S %d C %f",
            Double.valueOf(c.q),
            Integer.valueOf(c.sampleCount),
            Double.valueOf(c.cutoff)
          ),
          () -> {
            try {
              final var filter = new ARSV1Filter();
  ...
  private static void runFilter(
    ...
    final ARSV1Filter filter)
    throws Exception
  {
    final var name =
      "%s_q%.1fs%d_cutoff_%s.properties"
        .formatted(
          "svbrf",
          Double.valueOf(c.q),
          Integer.valueOf(c.sampleCount),
    ...
    for (int index = 0; index < inputSample.frames(); ++index) {
      ...
      for (int s = 1; s <= c.sampleCount; ++s) {
        filter.processOneFrame(x);
      }
      outputBuffer.put(index, filter.bandRejectOutput());
    }
