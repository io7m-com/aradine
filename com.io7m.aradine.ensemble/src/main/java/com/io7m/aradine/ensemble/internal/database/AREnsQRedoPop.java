...
/**
 * Pop a record from the redo stack.
 */

public enum AREnsQRedoPop
  implements AREnsQRedoPopType, ARDBQueryProviderType
{
  /**
   * Pop a record from the redo stack.
   */
  ...
  private static final String QUERY_TEXT = """
    DELETE FROM redo
      WHERE redo_id = (SELECT max(redo_id) FROM redo)
        RETURNING redo_id, redo_data_type, redo_data
    """;

  @Override
  public Class<?> queryInterface()
  {
    return AREnsQRedoPopType.class;
  }

  @Override
  public AREnsQRedoPopType queryInstance()
  {
    return this;
  }
    ...
    throws ARDBException
  {
    ...
    try (var st = connection.prepareStatement(QUERY_TEXT)) {
      try (var rs = st.executeQuery()) {
        while (rs.next()) {
          return Optional.of(AREnsUndoRedo.readRedoRecord(rs));
        }
